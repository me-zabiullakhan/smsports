                ...
                <div className="absolute bottom-4 left-4 w-48 md:w-56 h-56 md:h-64 z-20 drop-shadow-[0_10px_20px_rgba(0,0,0,0.5)] transition-transform duration-500 border-4 border-white bg-gray-800 rounded-xl overflow-hidden">
                    ...
                    {status === 'SOLD' && (
                        <div className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-[2px]">
                            <div className="bg-green-600 text-white font-black text-2xl md:text-3xl px-4 py-2 border-4 border-white -rotate-12 shadow-xl animate-bounce-in uppercase tracking-wider">
                                SOLD
                            </div>
                        </div>
                    )}
                    {status === 'UNSOLD' && (
                         <div className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-[2px]">
                            <div className="bg-red-600 text-white font-black text-2xl md:text-3xl px-4 py-2 border-4 border-white -rotate-12 shadow-xl animate-bounce-in uppercase tracking-wider">
                                UNSOLD
                            </div>
                        </div>
                    )}
                ...
                {bidder ? (
                    <div className="flex items-center gap-3 md:gap-6 z-10 text-right animate-fade-in w-full justify-end">
                        <div className="min-w-0 flex-1 flex flex-col items-end">
                            <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-wider mb-1 whitespace-nowrap">
                                {status === 'SOLD' ? 'SOLD TO' : 'WINNING BIDDER'}
                            </p>
