...
const PlayerPool: React.FC = () => {
  ...
  const soldPlayerIds = useMemo(() => 
    new Set(teams.flatMap(team => team.players.map(p => p.id))), 
    [teams]
  );
  ...
  const filteredPlayers = useMemo(() => {
    // Fix: Use Omit<Player, 'status'> to allow status to be strictly 'upcoming' | 'sold'
    let list: (Omit<Player, 'status'> & { status: PlayerStatus, soldTo?: string, teamLogo?: string })[] = [];

    if (activeTab === 'upcoming') {
        // Upcoming are players in the pool who are NOT in the sold set
        list = unsoldPlayers
            .filter(p => !soldPlayerIds.has(p.id))
            .map(p => ({ ...p, status: 'upcoming' as PlayerStatus }));
  ...
  }, [activeTab, players, teams, soldPlayerIds, unsoldPlayers, searchTerm, selectedCategory]);

  const TabButton: React.FC<{tab: PlayerStatus, label: string, icon: React.ReactNode}> = ({tab, label, icon}) => (
      <button 
        onClick={() => setActiveTab(tab)}
        className={`flex-1 py-3 text-sm font-bold flex items-center justify-center gap-2 transition-all duration-300 border-b-2 ${
            activeTab === tab 
        ...
        <div className="space-y-3">
            ...
            <div className="relative group">
                ...
                <input 
                    ...
                    className="w-full bg-primary border border-gray-600 rounded-lg py-2.5 pl-10 pr-10 text-text-main text-sm focus:outline-none focus:ring-1 focus:ring-highlight focus:border-highlight transition-all"
                />
                {searchTerm && (
                    <button 
            ...
            <div className="relative">
                ...
                <select
                    ...
                    onChange={(e) => setSelectedCategory(e.target.value as PlayerCategory | 'All')}
                    className="w-full bg-primary border border-gray-600 rounded-lg py-2.5 pl-10 pr-8 text-text-main text-sm focus:outline-none focus:ring-1 focus:ring-highlight focus:border-highlight appearance-none cursor-pointer transition-all"
                >
            ...
            <div className="space-y-2">
                {filteredPlayers.map(player => (
                    <div 
                        key={player.id} 
                        className={`
                            p-3 rounded-lg flex items-center gap-3 transition-all border
                            ${player.id === currentPlayer?.id && activeTab === 'upcoming'
                                ? 'bg-highlight/10 border-highlight shadow-[0_0_10px_rgba(56,178,172,0.2)]' 
                                : 'bg-primary/40 border-transparent hover:border-gray-600 hover:bg-primary/60'}
                        `}
                    >
                        <img 
                            src={player.photoUrl} 
                        ...
                        <div className="flex-grow min-w-0">
                            ...
                            <p className="text-xs text-text-secondary truncate">{player.category}</p>
                        </div>
                        
                        <div className="text-right">
                            {player.status === 'upcoming' ? (
                                <span className="inline-block px-2 py-1 bg-gray-700 rounded text-xs text-gray-300 font-mono">
                                    {player.basePrice}
                                </span>
                            ) : (
