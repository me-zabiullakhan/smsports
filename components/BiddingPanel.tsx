...
const BiddingPanel: React.FC = () => {
  ...
  const { currentBid, teams, highestBidder, biddingEnabled } = state;
  ...
  const isLoadingBid = nextBid === 0; // Safeguard against initial 0 display

  const handleBid = async () => {
    if (canAfford && !isLeading && biddingEnabled) {
      setIsBidding(true);
      try {
  ...
  return (
    <div className="bg-secondary rounded-xl shadow-xl p-3 md:p-6 border border-highlight/30 relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <button
          onClick={handleBid}
          disabled={!canAfford || isLeading || isBidding || !biddingEnabled || isLoadingBid}
          className={`
      ...
      {!canAfford && biddingEnabled && <p className="text-red-400 text-xs mt-2 flex items-center justify-center sm:justify-start font-bold"><AlertCircle className="w-3 h-3 mr-1"/> Insufficient Budget</p>}
      {!biddingEnabled && <p className="hidden sm:flex text-red-300 text-xs mt-2 items-center justify-center sm:justify-start font-bold uppercase tracking-wide"><Lock className="w-3 h-3 mr-1"/> Bidding Paused by Admin</p>}
    </div>
  );
};
